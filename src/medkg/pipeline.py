...
from medkg.config import CODED_TYPES, ASSERTION_TYPES, ALLOWED_ASSERTIONS, ALLOWED_TYPES, get_paths, get_settings
from medkg.context import ContextDetector
from medkg.io import discover_input_files, output_path_for, read_text, write_output
from medkg.models.client import LocalLLMClient
from medkg.models.prompts import SYSTEM_PROMPT, build_decision_prompt
from medkg.ner import MedicalNER
from medkg.ontology import OntologyIndex, load_ontology_index
from medkg.reasoning import infer_relations
from medkg.retrieval import CandidateRetriever
...
class MedicalKGPipeline:
    def __init__(self, index: OntologyIndex | None = None, root: Path | None = None):
        ...
        self.retriever = CandidateRetriever(self.index)
    ...
    def _apply_llm_decisions(
        ...
        for idx, concept in enumerate(concepts):
            ...
            candidate_entries = self.index.lookup(concept.text, concept.type, limit=5) if concept.type in CODED_TYPES else []
            candidate_rows = [
...
def main(argv: list[str] | None = None) -> int:
    paths = get_paths()
    parser = argparse.ArgumentParser(description="Run medkg inference")
