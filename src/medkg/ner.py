...
from medkg.normalization import normalize_key, trim_span_text
...
DIAG_CONTEXT_RE = re.compile(
    r"(?:ch\u1ea9n\s*\u0111o\u00e1n|chan\s*doan|diagnosis|dx)\s*(?:m\u1eafc|mac|l\u00e0|la|:)?\s*(?P<diag>[^.;\n]+)",
    re.IGNORECASE,
)
...
class MedicalNER:
    ...
    def _extract_lab_pairs(self, text: str) -> list[SpanCandidate]:
        ...
        for match in LAB_PAIR_RE.finditer(text):
            ...
            if _looks_like_lab_name(name_text):
        ...
        for match in DIAG_CONTEXT_RE.finditer(text):
            ...
            for prefix in ("m\u1eafc ", "mac "):
                if normalize_key(raw).startswith(normalize_key(prefix)):
                    ...
                    break
            start, end, span_text = trim_span_text(text, start, end)
            if span_text and len(span_text) >= 4:
...
def _extend_drug_end(text: str, offset: int) -> int:
    ...
    match = re.match(
        ...
        r"\d+(?:[.,-]\d+)?|mg/ml|mcg/ml|mg/dl|mg|mcg|g|ml|po|iv|im|sc|bid|tid|qid|qam|qhs|q\d+h:?prn|daily|prn|\+))+",
        candidate,
        flags=re.IGNORECASE,
    )
    if match:
